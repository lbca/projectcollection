(function(window, undefined){
	...
	function getTranslate(x, y){
		var distX = x, distY = y;
		return (support.transform3d ? "translate3d("+ distX +"px, "+ distY +"px, 0)" : "translate("+ distX +"px, "+ distY +"px)") + " scale(" + imgScale + ")";
	}
			...
			for(var len=imgList.length,i=0; i<len; i++){
				imgList[i].addEventListener("click", function(){
					...
					zoomImg.onload = function(){
						...
						// 没有屏幕宽的计算缩小值
						if(zoomImg.offsetWidth < zoomMask.offsetWidth) {
							imgScale = (zoomImg.offsetWidth/zoomMask.offsetWidth).toFixed(2);
						}
						cssText = "visibility: visible;width: 100%;-webkit-transform: scale(" + imgScale + ");";
						zoomImg.style.cssText = cssText;

						// 垂直居中
						imgMT = (zoomMask.offsetHeight-zoomImg.offsetHeight)/2;
						cssText += "margin-top:"+imgMT+"px;";
						zoomImg.style.cssText = cssText;
