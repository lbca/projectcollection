...
var NeiGouDistrict = function(opt) {
    this.options = $.extend({
        sel: '',
...
$.extend(NeiGouDistrict.prototype, {
    /**
     * 初始化
     */
    init: function() {
    ...
    renderItem: function(arr) {
        ...
        for(var i = 0, j = arr.length; i < j; i++) {
            html += '<li val="' + arr[i].C + '" fn="' + arr[i].F + '">' + arr[i].N + '</li>';
        }
        $('#list').html(html);
    },
    /**
    ...
    save: function() {
        var that = this,
            ...
            fullName = that.options.dstrict.fn,
            // 业务类型
            b = hrefParma.b,
            // 地区详情
            parma = {
        ...
        // 拼接参数
        hrefParma.gname && (parma.gname = hrefParma.gname);
        hrefParma.mob && (parma.mob = hrefParma.mob);
        hrefParma.refid && (parma.refid = hrefParma.refid);
        hrefParma.foi && (parma.foi = hrefParma.foi);
    ...
    addEvent: function() {
        ...
        that.el.on(click, '#list li', function() {
            ...
            // 全名
            that.options.dstrict.fn = $(this).attr('fn') || '';
